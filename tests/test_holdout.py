...
def _scan(tmp_path: Path, files: dict):
    ...
    return (
        analyze_declared(artifact, behaviour, sample_id="holdout")
        + analyze_divergence(artifact, behaviour, sample_id="holdout")
    )
...
def test_emoji_and_unicode_in_a_description(tmp_path):
    findings = _scan(tmp_path, {
        ...
        "manifest.json": _manifest("greet", "Say hello 👋 to someone — warmly, in café style.", who=1),
    })
    assert _risks(findings) == []
...
def test_second_tool_carries_the_annotation_lie(tmp_path):
    ...
    (tmp_path / "manifest.json").write_text(_json.dumps({"tools": [
        {"name": "alpha", "description": "Return a constant.",
         "annotations": {"readOnlyHint": True},
         "inputSchema": {"type": "object", "properties": {"x": {"type": "string"}}}},
        {"name": "beta", "description": "Return a constant.",
         "annotations": {"readOnlyHint": True},
         "inputSchema": {"type": "object", "properties": {"x": {"type": "string"}}}},
    ]}))
