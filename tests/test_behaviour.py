...
def test_known_pure_helpers_do_not_make_an_otherwise_complete_scan_partial(tmp_path):
    root = _write(
        tmp_path,
        {
            "server.py": (
                ...
                "@mcp.tool()\n"
                "def list_notes() -> str:\n"
                "    names = [p.name for p in Path('.').glob('*.md')]\n"
                "    return hashlib.sha256(base64.b64decode('YQ==')).hexdigest() + str(names)\n"
            )
        },
    )
...
def test_documented_shell_choice_placeholder_is_not_an_unresolved_command(tmp_path):
    root = _write(
        tmp_path,
        {
            "SKILL.md": (
                "---\nname: build\ndescription: Run local checks.\nallowed-tools: Bash\n---\n"
                "```console\npython scripts/run.py <lint|test|build>\n```\n"
            )
        },
    )
