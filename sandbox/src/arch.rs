...
pub fn read_sockaddr(pid: pid_t, addr: u64, len: usize) -> Option<String> {
    ...
    let local = iovec { iov_base: buf.as_mut_ptr() as *mut c_void, iov_len: size };
    let remote = iovec { iov_base: addr as *mut c_void, iov_len: size };
    ...
    match family as i32 {
        libc::AF_INET if size >= 8 => {
            ...
            Some(format!("{}.{}.{}.{}:{}", buf[4], buf[5], buf[6], buf[7], port))
        }
        ...
        libc::AF_UNIX => {
            ...
            Some(format!("unix:{}", String::from_utf8_lossy(&buf[2..2 + end])))
        }
